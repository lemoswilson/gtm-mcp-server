...
func TestNewServiceAccountTokenSource_KeyFile(t *testing.T) {
	keyJSON := testServiceAccountJSON(t)
	f, err := os.CreateTemp(t.TempDir(), "sa-key-*.json")
	if err != nil {
		t.Fatalf("create temp file: %v", err)
	}
	if _, err := f.Write(keyJSON); err != nil {
		t.Fatalf("write key file: %v", err)
	}
	f.Close()

	ts, err := NewServiceAccountTokenSource(context.Background(), "", f.Name())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if ts == nil {
...
func TestNewServiceAccountTokenSource_KeyFileMissing(t *testing.T) {
	_, err := NewServiceAccountTokenSource(context.Background(), "", "/nonexistent/path/key.json")
	if err == nil {
